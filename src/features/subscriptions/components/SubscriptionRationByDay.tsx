...
import { SubscriptionRationWeekGrid } from '@/features/subscriptions/components/SubscriptionRationWeekGrid'
    ...
    <section className="mb-4 rounded-[var(--radius-large)] border border-[color:var(--stroke)] bg-[color:var(--surface)] px-3 py-3 shadow-[var(--shadow-soft)]">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[15px] font-extrabold">рацион на неделю</p>
          {summaryLine ? <p className="mt-0.5 text-[12px] text-[color:var(--muted)]">{summaryLine}</p> : null}
          <p className="mt-1 text-[10px] text-[color:var(--muted)]">з · о · у — завтрак, обед, ужин</p>
        </div>
