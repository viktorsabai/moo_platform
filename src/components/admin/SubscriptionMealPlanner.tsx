...
export function SubscriptionMealPlanner({ config, dishes, onClose }: { config: SubscriptionConfig; dishes: PlannerDish[]; onClose: () => void }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        // explicit server save is intentionally separate from local draft
      }
    }, 500)
  ...
  return (
    <section id="subscription-meal-planner" className="scroll-mt-24 rounded-[28px] border border-[color:var(--primary)]/20 bg-[color:var(--surface-strong)] p-5 shadow-[var(--shadow-card)]">
      ...
      {restored ? <div className="mt-3 rounded-2xl bg-[color:var(--primary)]/[0.08] px-3 py-2 text-[12px] font-bold text-[color:var(--text)]">восстановлен черновик рациона</div> : null}
      <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
        ...
        <div className="text-left sm:text-right"><p className="text-[11px] font-extrabold uppercase tracking-wide text-[color:var(--muted)]">черновик</p><p className="mt-1 text-[13px] font-extrabold text-[color:var(--text)]">{savedAt ? `сохранён в ${savedAt}` : 'сохраняется автоматически'}</p></div>
      </div>
