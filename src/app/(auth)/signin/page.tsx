...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    const inWebApp = Boolean(w?.Telegram?.WebApp)
    if (inWebApp) {
      router.replace('/')
      return
    }
  ...
  return (
    <main className="mx-auto w-full max-w-[420px] px-4 pt-5 pb-28">
      ...
      <section className="rounded-[18px] border border-black/10 bg-white/70 p-4">
        ...
        <p className="mt-1 text-[13px] text-black/45">Нажмите кнопку ниже — откроется бот или mini app.</p>
        {tmeLink && (
          <a
            href={tmeLink}
        ...
        <Link
          href="/"
          prefetch={false}
          className="mt-3 inline-flex h-11 w-full items-center justify-center rounded-full border border-black/10 bg-transparent px-6 text-[13px] font-semibold text-black/70"
        >
          На главную
        </Link>
      </section>
