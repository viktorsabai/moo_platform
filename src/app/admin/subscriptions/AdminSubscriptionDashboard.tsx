...
export function AdminSubscriptionDashboard() {
  ...
  const [guestLimitsOpen, setGuestLimitsOpen] = useState(false)
  ...
  return (
    <main className="ui-container flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden pt-1">
      <header className="shrink-0 space-y-2 pb-2">
        <AdminSubscriptionNav />
        <p className="ui-muted text-[12px] leading-snug">
          Слот → категория → отметьте блюда. Лимиты — на категорию, по умолчанию безлимит.
        </p>
      </header>

      {/* Слот — главная навигация */}
      <section className="shrink-0 pb-2">
        <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-[color:var(--muted)]">
          слот доставки
        </p>
        ...
        <p className="ui-muted mt-1.5 text-[11px]">
          {slotItemCount === 0
            ? `В ${MEAL_SLOT_LABEL[activeSlot]} пока пусто — отметьте блюда ниже`
            : `${slotItemCount} поз. · лимит слота: ${slotMaxLabel(slotConfig.maxItemsPerDelivery)}`}
        </p>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-semibold text-[color:var(--muted)]">макс. в слоте за доставку</span>
          <button
            type="button"
      ...
      <section className="shrink-0 pb-2">
        <div className="mb-1.5 flex items-center justify-between gap-2">
          ...
          {categoryFilter !== ALL_FILTER && filteredProducts.length > 0 ? (
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => bulkToggleCategory(true)}
              ...
              <button
                type="button"
                onClick={() => bulkToggleCategory(false)}
        ...
        {categoryFilter !== ALL_FILTER && categoryFilter !== OPTIONS_FILTER ? (
          <div className="mt-2 flex flex-wrap items-center gap-2 rounded-xl bg-black/[0.03] px-3 py-2">
            <span className="text-[11px] font-semibold text-[color:var(--muted)]">
              лимит «{activeCategoryLabel}» за доставку
            </span>
            <button
              type="button"
      ...
      {/* Сетка блюд — как в меню, 2 колонки */}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {filteredProducts.map((p) => {
              ...
              return (
                ...
                >
                  <div className="relative h-44 w-full overflow-hidden bg-[color:var(--surface-strong)]">
                    {p.image ? (
                      <OptimizedImage
                        src={p.image}
                        alt=""
      ...
      {/* Маржа — 1 доставка по активному слоту */}
      <section
        className="ui-surface-strong shrink-0 border-t border-[color:var(--stroke)] p-4"
        style={{ borderRadius: 'var(--radius-large) var(--radius-large) 0 0' }}
      >
        {slotItemCount === 0 ? (
          <p className="ui-muted text-[13px]">
            Добавьте блюда в <strong>{MEAL_SLOT_LABEL[activeSlot]}</strong> — покажем цену одной доставки.
          </p>
        ) : quote ? (
          <div className="space-y-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-[color:var(--muted)]">
              пример · 1 доставка ({MEAL_SLOT_LABEL[activeSlot]})
            </p>
            <div className="grid grid-cols-2 gap-2 text-[12px]">
              <div className="rounded-xl bg-black/[0.03] p-2.5">
                <div className="text-[color:var(--muted)]">розница</div>
                <div className="text-[18px] font-extrabold tabular-nums">{formatPrice(perDeliveryRetail)}</div>
              </div>
              <div className="rounded-xl bg-black/[0.03] p-2.5">
                <div className="text-[color:var(--muted)]">подписка</div>
                <div className="text-[18px] font-extrabold tabular-nums text-[color:var(--primary)]">
                  {formatPrice(perDeliveryGuest)}
                </div>
              </div>
            </div>
            ...
            {perDeliveryProfit != null ? (
              <p className="text-[12px] text-[color:var(--muted)]">
                прибыль ~ <strong className="text-[color:var(--text)]">{formatPrice(perDeliveryProfit)}</strong> за доставку
                {quote.ownerMarginPercent > 0 ? ` · маржа ${Math.round(quote.ownerMarginPercent)}%` : ''}
              </p>
            ) : (
              <p className="text-[12px] text-amber-700">
                Заполните себестоимость — покажем точную маржу
                {quote.missingCostCount > 0 ? ` (${quote.missingCostCount} без себест.)` : ''}.
              </p>
            )}

            {perDeliveryRetail > perDeliveryGuest ? (
              <span className="inline-block rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800">
                −{Math.round(((perDeliveryRetail - perDeliveryGuest) / perDeliveryRetail) * 100)}% vs розница
              </span>
            ) : null}
            ...
            {manualPriceOpen ? (
              <label className="block">
                <span className="text-[11px] font-semibold text-[color:var(--muted)]">
                  фикс. цена за период ({config.defaultPeriodDays} дн.), ฿
                </span>
                <input
                  type="number"
                ...
                {manualPriceInvalid ? (
                  ...
                  <p className="mt-1 text-[11px] text-[color:var(--muted)]">Пусто — цена считается автоматически из маржи и скидки.</p>
                )}
              </label>
            ) : (
              <button
            ...
            <button
              type="button"
              onClick={() => setGuestLimitsOpen((v) => !v)}
              className="flex w-full items-center justify-between text-[12px] font-semibold text-[color:var(--muted)]"
            >
              <span>лимиты для гостя</span>
              <span>{guestLimitsOpen ? '▲' : '▼'}</span>
            </button>

            {guestLimitsOpen ? (
              <div className="space-y-3 rounded-xl bg-black/[0.03] p-3 text-[12px]">
                <p className="text-[11px] text-[color:var(--muted)]">
                  Гость сам выбирает дни доставки в этих рамках. Вам не нужно настраивать «сколько дней» на каждом блюде.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <label>
                    <span className="text-[10px] font-semibold uppercase text-[color:var(--muted)]">мин. персон</span>
                    <input
                      type="number"
                      min={1}
                      max={10}
                      ...
                      onChange={(e) =>
                        patchConfig({
                          ...config,
                          minPersons: Math.max(1, Math.min(10, Number(e.target.value) || 1)),
                        })
                      }
                      className="input mt-1 w-full rounded-lg px-2 py-1.5 text-[13px] font-semibold tabular-nums"
                    />
                  ...
                  <label>
                    <span className="text-[10px] font-semibold uppercase text-[color:var(--muted)]">макс. персон</span>
                    <input
                      type="number"
                      min={1}
                      max={10}
                      ...
                      onChange={(e) =>
                        patchConfig({
                          ...config,
                          maxPersons: Math.max(config.minPersons, Math.min(10, Number(e.target.value) || 10)),
                        })
                      }
                      className="input mt-1 w-full rounded-lg px-2 py-1.5 text-[13px] font-semibold tabular-nums"
                    />
