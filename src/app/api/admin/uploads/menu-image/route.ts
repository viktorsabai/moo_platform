...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (process.env.BLOB_READ_WRITE_TOKEN) {
      const blob = await put(`menu/${ctx.restaurantId}/${fileName}`, buffer, {
        access: 'public',
    ...
    const relativeDir = path.join('uploads', 'menu', ctx.restaurantId)
    ...
    return NextResponse.json({
      ok: true,
      url: `/${path.posix.join('uploads', 'menu', ctx.restaurantId, fileName)}`,
    })
  } catch (e: any) {
    const status = Number(e?.statusCode || 500)
    const code = String(e?.code || '').toUpperCase()
    if (code === 'EROFS' || code === 'EACCES' || code === 'EPERM') {
      return NextResponse.json({ ok: false, error: 'Настройте BLOB_READ_WRITE_TOKEN для загрузки фото' }, { status: 503 })
    }
    if (code === 'ENOENT') {
      return NextResponse.json({ ok: false, error: 'Настройте BLOB_READ_WRITE_TOKEN для загрузки фото' }, { status: 503 })
    }
